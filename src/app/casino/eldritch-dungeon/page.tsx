...
const ENEMIES: Record<EnemyType, { name: string; emoji: string; hp: number; reward: string; symbolTransform: number; wildAdd: number }> = {
  ...
  nightfang: { name: 'Nightfang', emoji: 'F', hp: 40, reward: '4 Wilds added', symbolTransform: 0, wildAdd: 4 },
  elderfang: { name: 'Elderfang', emoji: 'L', hp: 60, reward: '10 Wilds added', symbolTransform: 0, wildAdd: 10 }
}
...
export default function EldritchDungeon() {
  ...
  const startBattle = useCallback(() => {
    ...
    setBattleLog([`Battle: ${enemyConfig.name} appeared!`])
    setShowBattle(true)
    setGameState('battle')
  ...
  const checkOldOneTrigger = useCallback(() => {
    ...
    if (unlockedCount >= 40 || Math.random() < 0.2) {
      setFightingOldOne(true)
      setMessage('THE OLD ONE AWAKENS!')
      
      setTimeout(() => {
        setBonusMultiplier(2)
        const bonusWin = bonusTotal * 2 + betAmount * 100
  ...
  const buyFeature = useCallback((multiplier: number) => {
    const costs: Record<number, number> = { 1: 17.3, 2: 45.5, 3: 83.6, 4: 143.7, 5: 218 }
    const cost = (costs[multiplier] || 17.3) * betAmount
  ...
  const renderSymbol = (cell: GridCell) => {
    ...
    return (
      <div 
        className={`
          w-full h-full rounded-lg flex items-center justify-center text-base sm:text-lg md:text-xl font-bold
        ...
        `}
      >
        <span className="drop-shadow-lg relative z-10 text-white">{config.emoji}</span>
    ...
    return (
      <div className={`bg-gradient-to-br ${hero.color} rounded-xl p-2 border border-gray-700/50`}>
        <div className="flex items-center gap-2">
          <span className="text-2xl font-bold text-white">{hero.emoji}</span>
        ...
        <div className="grid grid-cols-4 gap-1 mt-2 text-[10px]">
          ...
          <div className="text-center bg-black/30 rounded p-1">
            <div className="text-yellow-400">CRT</div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-950 via-slate-900 to-gray-950 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {/* Grunge texture over background */}
        <div className="absolute inset-0 pointer-events-none opacity-10 mix-blend-multiply" aria-hidden="true">
          {/* Background PNG */}
          <div className="w-full h-full bg-[url('/casino/bg-grunge.png')] object-cover" />
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-b from-gray-900 to-gray-950 rounded-2xl p-6 max-w-md w-full border border-purple-500/50">
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="text-center">
                <div className="text-4xl mb-2 font-bold text-white">{HEROES[selectedHero].emoji}</div>
              ...
              <div className="text-center">
                <div className="text-4xl mb-2 font-bold text-white">{ENEMIES[currentEnemy].emoji}</div>
          ...
          <div className="bg-gradient-to-b from-gray-900 to-gray-950 rounded-2xl p-4 max-w-md w-full border border-purple-500/50" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-3 gap-3">
              {(Object.entries(HEROES) as [HeroType, typeof HEROES.arcanist][]).map(([key, hero]) => (
                ...
                >
                  <div className="text-3xl mb-1 font-bold text-white">{hero.emoji}</div>
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto">
              ...
              <div className="border-t border-gray-700 pt-2 mt-2">
                ...
                <div className="text-xs text-gray-400 space-y-1">
                  <p>Cascading Wins: Winning symbols vanish and new ones drop</p>
                  <p>Dungeon Battle: Fight enemies when Warrior appears next to wins</p>
                  <p>Treasure Hall: Hold and Win bonus with keys and coins</p>
                  <p>The Old One: Final boss awards 4x4 Colossal Coin + 2x Multiplier</p>
                </div>
              </div>
